...
def grader(summary,feedback, answer1, answer2):

    prompt=    prompt = f"""[Summary]
    {summary}
    [Human Feedback]
    {feedback}
    ...
    We would like to request your feedback on the performance of two AI assistants in giving suggestions on next task to perform for the research.
    In particular, both AI assistants are giving advice to a human apprentice in a research project to develop a novel aluminum MOF using Benzene Tribenzoate (BTB) as a linker. 
    The project is structured into five stages:

    1) Synthesis of Organic Linker (BTB).
    2) High-throughput screening of the MOF and optimization of the synthetic outcomes via PXRD.
    3) Activation and Determination of Permanent Porosity.
    4) Detailed Structural Analysis and Characterization of the MOF for single crystal structure, chemical composition, phase purity, and chemical and thermal stability.
    5) Reproducibility Check and Final Validation
    ...
    You job is to look in to the answer gave by the two assistants, and rate the relevance, accuracy and helpfulness of their responses.
    Below are the rules:
    
    Relevance (to Previous Tasks):
    Question: Does the suggested task directly build upon or is it coherent with the most recent results, observations, and feedback provided?
    ...
    Description: This metric embodies the concept of "accuracy" in research progression. 
    Every task should either improve current methods or initiate a meaningful leap forward. 
    Suggestions that don't fit the logical evolution of the study or seem redundant may not be as accurate in guiding the next steps.
    The task should be fitted within the context of each stage. For example, in stage 4 Detailed Structural Analysis and Characterization, there should not be any tasks that go back or are related to high throughput synthesis, which should have been done in stage 2.
    Similar, in stage 2 MOF optimization, there should not be tasks related to activation and porosity measurement in stage 3.
    
    Helpfulness:
    Question: Is the task clear, detailed, and practical to execute, aiding the researcher in their next steps?
    Description: Similar to the "Feasibility and Clarity" perspective, this metric ensures tasks are not just theoretically sound but also actionable. A task that aligns well with the study's objectives but lacks clarity or practicality might not be as helpful in guiding the research.

    You should not consider the level of details or length in the response because the task suggestions was designed to provide direction rather than step by step instruction. 
    
    
    Each assistant has three task suggestions, and each task suggestion receives a score of 1 or 0, where 1 indicates the criteria was met.
    Therefore, for each assistant's answer, the total score is 9, which is the sum of its three task's score
    Please first provide a comprehensive explanation of your evaluation, avoiding any potential bias and ensuring that the order in which the responses were
    presented does not affect your judgment. Then, output two lines indicating the scores for Assistant 1 and 2, respectively.
    Output with the following format:
    **
    Evaluation evidence: <evaluation explanation here>
    The score of Assistant 1: <total score, max 9> <score of task 1> <score of task 2>  <score of task 3>
    The score of Assistant 2: <total score> <score of task 1> <score of task 2>  <score of task 3>
    **
    
    The individual score of each task should be listed in the order of "Relevance", "Potential for Progress" and "Helpfulness"
    And the total score is the sum of all scores for all three tasks.
    For example:
    The score of Assistant 1: 8 1 1 0 1 1 1 1 1 1
    The score of Assistant 2: 2 0 1 0 0 0 0 1 0 0 
...
def grade_assistants(assistant1, assistant2, start_row=0, end_row=None):
    # Mapping assistant names to corresponding columns
    assistant_columns = {
        'A': 'Full prompt',
        'B': 'Prompt without role and goal',
        'C': 'Prompt without memory',
        'D': 'Prompt without evaluation',
    }
    

    # Loading the Excel file into a DataFrame
    data = pd.read_excel("output_data.xlsx", sheet_name="Sheet1")



    # Checking if the provided assistant names are valid
    if assistant1 not in assistant_columns or assistant2 not in assistant_columns:
        print("Invalid assistant names. Please choose from 'A', 'B', 'C', or 'D'.")
        return

    # Subsetting the data if a specific range is provided
    if end_row is not None:
        rows_to_grade = data[start_row:end_row]
    else:
        rows_to_grade = data[start_row:]

    # Iterating through the rows and grading the assistants
    results = []
    for index, row in rows_to_grade.iterrows():
        summary = row['Summary']
        feedback = row['Human Feedback']
        answer1 = row[assistant_columns[assistant1]]
        answer2 = row[assistant_columns[assistant2]]

        # Calling the grader function and storing the result
        result = grader(summary, feedback, answer1, answer2)
        results.append(result)

        #print(f"Graded row {index + 1} - Result: {result}")
